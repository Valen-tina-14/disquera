    ...
    <br><br><br>
     ...
     <section  id="formulario">
        ...
        <c:forEach var="albunes" items="${albunes}">         
          ...
          <tr>
          <td>${albunes.getIdAlbum()}</td>
          <td>${albunes.getNombreAlbum()}</td>
          <td>${albunes.getAnioPublicacion()}</td>
          <c:if test="${albunes.isEstadoAlbum() == true}">
             <td><span>Álbum Activo</span></td> 
          </c:if>
          <c:if test="${albunes.isEstadoAlbum() == false}">
              <td><span>Álbum Inactivo</span></td> 
          </c:if>
          
          <td>
          <c:if test="${albunes.isEstadoAlbum() == true}">
             <a href="album?accion=esta&idAlbum=${albunes.getIdAlbum()}&estadoAlbum=false"><button>Inactivar</button>
        
          </a>
          </c:if>
          <c:if test="${albunes.isEstadoAlbum() == false}">
            <a href="album?accion=esta&idAlbum=${albunes.getIdAlbum()}&estadoAlbum=true"><button>Activar</button>

          </a>
          </c:if> 
              <td>
                <a href="album?accion=edit&idAlbum=${albunes.getIdAlbum()}"><button>Editar</button>
                  </a>

                  <a href="album?accion=eliminar&idAlbum=${albunes.getIdAlbum()}"><button>Eliminar</button>
                  </a>
                </c:forEach>  
</table>
  <center>
